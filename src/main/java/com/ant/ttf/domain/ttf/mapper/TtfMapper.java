...
public interface TtfMapper {
	...
	int regTtf(Ttf ttf); // 신파일러 적금가입 신청(ttf 테이블에 삽입)
	
	int regAcc(Account thinAcc); // 신파일러 적금가입 신청(account 테이블에 삽입)
	
	int seeLimit(String userPK); //userPK를 이용한 신파일러 계좌 한도조회
	
	int balUpdate(Map map); // 신파일러 결제
	
	int seeBalance(String userPK); //신파일러 계좌 잔액조회
	
	int seeLimitAmount(String userPK); //userPK가 아닌 account_id를 이용한 한도조회. 유저는 한명인데 계좌는 여러개일수도 있으니..
